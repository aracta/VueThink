<template>
    <el-dialog ref="dialog" :visible.sync="dialogVisible" custom-class="w-900 h-480 ovf-auto" title="节点列表">
        <div class="pos-rel h-60">
            <el-input placeholder="请输入内容" v-model="keyword" class="search-btn w-300" clearable>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      searchMsg() {
        this.tableData = this.tableDataShow.filter(data => !this.keyword || data.title.toLowerCase().includes(this.keyword.toLowerCase()))
      }
    },
    created() {
